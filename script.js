...
const main_btn = document.querySelector(".search-btn");
...
const callParams = () => {
    ...
    fetch(`https://jsearch.p.rapidapi.com/search?query= ${params} & page=1 & num_pages=1`, options)
        ...
        .then(items => {
            ...
            items.data.map(item => {
                ...
                const jobCard = `<li id="middle"><div class="essential_detail"><div class="company-logo-img">
                <img src="${logo}" alt=""></div>
                <h2 class="job_tite">${job_tit}</h2>
                <h1 class="company_name">${employ_name}</h2>
                <button class="job-btn" id="apply"><a href="${apply_link}" target="_blank">Apply</a></button>
                ...
                document.querySelector('.job_card').innerHTML += jobCard;
...
main_btn.addEventListener('click', callParams);
